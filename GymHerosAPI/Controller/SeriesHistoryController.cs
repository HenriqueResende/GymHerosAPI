    ...
    public class SeriesHistoryController : ControllerBase
    {
        ...
        public IActionResult Get(int? id)
        {
            try
            {
                ...
                var lstSeriesHistory = _BLSeriesHistory.Get(id.GetValueOrDefault());

                return Ok(new { Status = 200, SeriesHistory = lstSeriesHistory });
            }
            catch (Exception ex)
            {
        ...
        public IActionResult ListAll()
        {
            try
            {
                ...
                return Ok(new { Status = 200, SeriesHistorys = lstSeriesHistory });
            }
            catch (Exception ex)
            {
                LogError.SaveLog(ex, User.FindFirst(ClaimTypes.NameIdentifier)?.Value ?? "");
